...
// не проверяем правильность выделения памяти, так как это не относится к тесту.
    ...
    data_t *popped_data = pop_front(&head);

    ck_assert_ptr_eq(popped_data, &d_1);

    node_t *cur_head = head;

    for (int i = 1; cur_head != NULL; cur_head = cur_head->next, i++)
    {
...
Suite* check_pop_front(void)
{
    Suite *s;
    TCase *tc_pos;

    s = suite_create("check pop front");

    tc_pos = tcase_create("positives");
    tcase_add_test(tc_pos, test_pop_front_ok);
